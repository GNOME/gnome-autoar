                          ...
                          void           *client_data)
{
  ...
  g_debug ("libarchive_read_close_cb: extractor read %" G_GSSIZE_FORMAT, self->accumulated_read_size);
                         ...
                         int             whence)
{
  ...
  if (new_offset == 0) {
    g_debug ("libarchive_read_seek_cb: seeked %"G_GOFFSET_FORMAT, self->current_seek_offset);
    self->current_seek_offset = 0;
  } else if (self->current_seek_offset == 0) {
    self->current_seek_offset = new_offset;
  }
